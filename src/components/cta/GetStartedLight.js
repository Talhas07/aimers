...
const Subheading = tw.h6`text-gold-950 opacity-75`;
const Heading = tw.h5`text-gold-950`;
...
const PrimaryLink = tw(
  Link
)`shadow text-gray-100 hocus:text-gray-300 bg-gold-950 hocus:bg-orange-700`;

const SecondaryLink = tw(
  Link
)`text-gold-950 hover:text-orange-600 bg-gray-100 hover:bg-gray-200`;

const DecoratorBlobContainer = tw.div`absolute inset-0 overflow-hidden rounded-lg`;
const DecoratorBlob1 = tw(
  SvgDecoratorBlob1
)`absolute bottom-0 left-0 w-80 h-80 transform -translate-x-20 translate-y-32 text-gold-950 opacity-5`;
const DecoratorBlob2 = tw(
  SvgDecoratorBlob1
)`absolute top-0 right-0 w-80 h-80 transform  translate-x-20 -translate-y-64 text-gold-950 opacity-5`;
export default ({
