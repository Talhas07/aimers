...
const Card = styled.div`
  ${tw`flex flex-col mx-auto max-w-xs items-center px-6 py-10 border-2 border-dashed border-gold-950 rounded-lg mt-12`}
  .imageContainer {
    ${tw`border-2 border-gold-950 text-center rounded-full p-6 flex-shrink-0 relative`}
    img {
      ${tw`w-8 h-8`}
    }
  }
  ...
  .title {
    ${tw`mt-2 font-bold text-xl leading-none text-gold-950`}
  }
  ...
  return (
    <Container>
      <ThreeColumnContainer>
        <Heading>
          Our Professional <span tw="text-gold-950">Services</span>
        </Heading>
        {cards.map((card, i) => (
          <Column key={i}>
            <Card>
